...
def modify_date(data):
    ...
    for date in dates:
        ...
        new_datetime = years + "/" + lengthen(months) + "/" + lengthen(
            days) + " " + lengthen(hours) + ":" + lengthen(
                minutes) + ":" + seconds
...
def send_kinesis(kinesis_client, kinesis_stream_name, kinesis_shard_count,
                 data):
    ...
    for _, row in data.iterrows():
        ...
        values = '|'.join(
            str(value) for value in row)  # join the values together by a '|'
        ...
        kinesisRecord = {
            ...
            "PartitionKey":
            str(shardCount)  # some key used to tell Kinesis which shard to use
        }

        kinesisRecords.append(kinesisRecord)  # add the object to the list
        stringBytes = len(
            values.encode('utf-8'))  # get the number of bytes from the string
        currentBytes = currentBytes + stringBytes  # keep a running total

        # check conditional whether ready to send
        if len(kinesisRecords
               ) == 500:  # if we have 500 records packed up, then proceed
        ...
        if sendKinesis == True:
            ...
            response = kinesis_client.put_records(
                Records=kinesisRecords, StreamName=kinesis_stream_name)
...
def main():
    ...
    kinesis = create_client('kinesis', 'ap-southeast-2')
    ...
    stream_name = "testStream-bd64058"
