locals {
  autoscaling_group_name = aws_cloudformation_stack.dockerzon-cluster-asg.outputs["AsgName"]
  ecs_cluster_name       = aws_ecs_cluster.dockerzon.name
}
...
resource "aws_lambda_function" "container-draining_lambda" {
  ...
  function_name    = "container-draining-lambda"
  ...
  tags = {
    Name = "container-draining-lambda"
  }
}
...
resource "aws_iam_role_policy" "container-draining_lambda" {
  name = "container-draining-lambda-role-policy"
  role = aws_iam_role.container-draining-lambda-role.name
